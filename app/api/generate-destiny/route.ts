...
import { generateUserPrompt, getSystemInstruction, transformToLifeDestinyResult } from "@/lib/utils";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      !userInput.monthPillar ||
      !userInput.dayPillar ||
      !userInput.hourPillar
    ) {
    ...
    console.log("系统指令长度:", systemPrompt.length, "字符");
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt < MAX_RETRIES) {
          ...
          console.log(
            `API 返回 ${apiResponse.status}，${retryDelay}ms 后重试...`,
          );
        ...
        lastError =
          fetchError instanceof Error ? fetchError : new Error("未知网络错误");
    ...
    // 提取文本内容
    const contentBlock = anthropicData.content.find(
    ...
    // 提取 JSON 对象（找第一个 { 到最后一个 }，去除前后多余文字）
    ...
    // 转换为标准格式并标准化评分
    const transformedData: LifeDestinyResult =
      transformToLifeDestinyResult(rawData);
